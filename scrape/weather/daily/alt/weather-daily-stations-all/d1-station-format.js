...
const inputDir = path.join(__dirname, 'data/station-json-clean');
const outputDir = path.join(__dirname, 'data/station-json-formatted');
...
fs.readdirSync(inputDir).forEach(file => {
  if (path.extname(file) === '.json') {
    ...
    try {
      ...
      fs.writeFileSync(outputPathJson, JSON.stringify(filledData, null, 2), 'utf-8');
