...
function loadWeatherData(filePath) {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      .pipe(csv())
      .on('data', (row) => {
        const dateToCheck = moment(row['DateTime']);
        const targetDate = moment('2014-03-26');
...
function getTotalPrecipitationInRange(data, startDate, endDate) {
  const start = moment(startDate);
  const end = moment(endDate);
  return data
    .filter(d => {
      const dDate = moment(d.date);
      return dDate.isSameOrAfter(start) && dDate.isSameOrBefore(end);
    })
...
function findAllDrySpells(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].precipitation < 0.3) {
      currentSpell.push(data[i]);
    } else {
      if (currentSpell.length > 20) {
        const startDate = currentSpell[0].date;
        const endDate = currentSpell[currentSpell.length - 1].date;
        const totalPrecip = currentSpell.reduce((sum, d) => sum + d.precipitation, 0);

        const yearBeforeStart = moment(startDate).subtract(180, 'day');
        const precipBefore = getTotalPrecipitationInRange(data, yearBeforeStart, moment(startDate).subtract(1, 'day'));

        drySpells.push({
          startDate,
          endDate,
          length: currentSpell.length,
          totalPrecip: totalPrecip.toFixed(2),
          precipBefore: precipBefore.toFixed(2)
        });
      }
      currentSpell = [];
    }
